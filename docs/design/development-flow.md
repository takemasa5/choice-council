...
M3 では、前提整理で提示された専門家候補をユーザーが確定し、確定した最大5件の専門家コメントを並列生成する。全コメントの生成に成功した場合、ファシリテーターがコメントを整理して `direction` フェーズへ遷移する。専門家コメントの一部でも生成に失敗した場合は、成功分を表示・保持せず、ユーザー操作による全件再試行を待つ。
