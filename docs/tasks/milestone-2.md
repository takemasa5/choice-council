...
M2 では、ユーザーが相談内容を入力し、バックエンド経由で OpenAI API を呼び出し、ファシリテーターの初回応答を画面で確認できるところまでを対象とする。
...
| エラー | 入力不足、API key 未設定、OpenAI API 失敗、構造化出力不正の表示                                                  |
...
4. バックエンドは OpenAI API を呼び出し、`FacilitatorResponse` として検証する。
...
| OpenAI API 呼び出し失敗 | OpenAI API から受け取ったエラーメッセージをそのまま表示する |
...
- Given OpenAI API 呼び出しが失敗したとき、When バックエンドがエラーを受け取るなら、Then OpenAI API 由来のエラーメッセージを表示する。
- Given `OPENAI_API_KEY` が未設定のとき、When 初回応答 API を呼ぶなら、Then API key 未設定を説明するエラーを返す。
