...
保存済みの確定済み専門家候補が初期 LLM 候補と異なる、または生成済みコメントがその候補を参照する場合は、ユーザーの確定内容として非空の各フィールドを原文のまま復元する。初期 LLM 候補と同一の未編集確定候補は通常画面候補として正規化する。明示的な空配列ではなく現行の入力形式へ1件も復元できない場合は未保存として扱う。ファシリテーター応答の `expert_requests` は LLM 由来の通常画面候補として正規化し、確定済み候補を復元できない場合の選定候補に使う。明示的な空配列は保持する。

編集中の専門家候補を復元する場合、復元元候補キーは LLM 候補と同じ形式に正規化する。各下書きは同じ添字のキー候補と一致する未編集のものだけを LLM 由来として正規化し、異なる編集済み下書きは原文を維持する。これにより、同じ候補に対する編集途中の下書きは維持し、候補が変わった場合は新しい候補へ同期する。不正な復元元候補キーは保持しない。明示的な空候補のキーは有効な値として保持する。
...
保存済みの終了メモは、`現時点の状態` に単独の段落または順不同リスト項目として記載された終了状態が、保存済み `SessionMemo.status` と一致する場合だけ復元する。状態が欠ける、不一致、または他の終了状態が独立して併記される場合も、未生成と同様に `group_chat` へ戻して終了状態を選び直せるようにする。
