...
- クライアントが送信・保存する必須文字列は、検証と同じく trim 後の値を用いる。
...
専門家選定でユーザーが編集して確定する候補は LLM 通常画面出力ではない。`role_name`、`viewpoint`、`request` は非空文字列として保存・専門家コメント生成に渡し、改行、Markdown、150字超の内容も原文を保持する。保存時は `confirmedExperts` に分離し、ファシリテーター応答・履歴の `expert_requests` をユーザー編集値で上書きしない。
...
順不同リストの直後に1〜3個の半角空白で始まる本文行は、直前の項目の継続行として描画する。入れ子リストは扱わない。

`現時点の状態` には、リクエストの `memo.status` に対応する終了状態を、単独の段落または順不同リスト項目として必ず記載する。他の終了状態を単独の段落または順不同リスト項目として併記してはならない。補足説明では他の終了状態に言及してよい。
