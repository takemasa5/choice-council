...
保存済みの確定済み専門家候補が明示的な空配列ではなく、現行形式へ1件も正規化できない場合は未保存として扱う。ファシリテーター応答の `expert_requests` が復元可能なら、それを選定候補として復元する。明示的な空配列は保持する。

編集中の専門家候補を復元する場合、その復元元候補キーも候補本体と同じ形式に正規化する。これにより、同じ候補に対する編集途中の下書きは維持し、候補が変わった場合は新しい候補へ同期する。不正な復元元候補キーは保持しない。明示的な空候補のキーは有効な値として保持する。
